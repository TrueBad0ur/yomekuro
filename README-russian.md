...
На странице логина есть вкладка «Register» — любой, кто может достучаться до
сервера, получает не-админский аккаунт только для чтения; нормально для
приватной сети, но стоит иметь в виду перед тем как выставлять сервер в
открытый интернет.
...
По умолчанию открывается персональная вкладка **My Library**. Она группирует
серии в **Continue Reading** и **Completed**, показывает статистику чтения и
открывает незавершённую серию сразу на текущем или следующем непрочитанном
томе. Здесь же есть персональная категория **Read Later** и любые созданные
пользователем категории. Через меню **⋯** на тайтле или его томе вся серия
добавляется в категорию или удаляется из неё; пользовательские категории можно
переименовывать и удалять на странице My Library. Остальные вкладки переключают
на «All Titles» и отдельные библиотеки
(Books / Manga); клик по серии показывает её книги, клик по книге открывает
чтение. Поиск и фильтры по жанрам/тегам доступны в представлениях каталога.

Кнопка **Cache: On / Off** управляет браузерным кэшем обложек и страниц
читалки. Для обычного использования оставьте кэш включённым; после повторной
конвертации тома временно выключите его, чтобы все изображения и страницы
загрузились заново.
...
![Персональная библиотека с Read Later и Continue Reading](docs/screenshots/library-personal-loaded-dark.png)

При открытии серии манги сетка тайтлов сменяется томами в порядке чтения, с
прогрессом и быстрыми действиями для каждого тома.

![Тома ぐらんぶる в актуальном интерфейсе](docs/screenshots/grand-blue-volumes-current-dark.png)
...
![Читалка](docs/screenshots/grand-blue-reader.png)

### Закладки

Выделите текст во время чтения, чтобы создать закладку: можно выбрать один из
четырёх цветов и добавить заметку. В одной книге может быть сколько угодно
закладок; панель **Bookmarks** позволяет просмотреть их, перейти к отмеченной
странице, изменить цвет или заметку и удалить ненужное. Закладки видны только
текущему пользователю и не ломаются на страницах с фуриганой
(`<ruby>`/`<rt>`), так как в тег оборачиваются только отдельные текстовые узлы,
а не целые элементы.

![Несколько цветных закладок](docs/screenshots/bookmarks-multicolor-dark.png)
...
![Страница настроек](docs/screenshots/settings-libraries-dark.png)
...
завершённые — убрать из списка). **Pause Queue** / **Resume Queue** ставят на
паузу все задачи в очереди, кроме той, что конвертируется прямо сейчас (она
доходит до конца) — в отличие от Stop, пауза никогда не трогает файлы, так
что всю очередь можно оставить на часы (например, дать хосту остыть) и потом
продолжить ровно с того же места.

Исходный, ещё не прошедший OCR скан каждой загрузки также зеркалируется в
`./backup/<library>/<name>/`, если задана папка бэкапа (`YOMEKURO_BACKUP_DIR`,
по умолчанию смонтирована в обоих compose-файлах) — это подстраховка,
независимая от пайплайна конвертации: OCR можно перезапустить из сырого
скана, а вот потерянный сырой скан не восстановить.

![Загрузка и очередь конвертации](docs/screenshots/upload-jobs-dark.png)
...
- **Reconvert (full OCR)** — полностью перезапускает OCR с нуля (не
  пересборку из кэша) для одного тома или всей книги, с выбором разрешения
  текстового детектора (2048/3072/3584 — чем выше, тем больше распознаёт, но
  и больше нужно видеопамяти). Доступно, только пока на диске ещё есть
  исходный скан книги (`<name>-in/`) — иначе показывается «no raw scan».
  Бейдж **⚠ raw scan changed** появляется на любом томе, чьи файлы сырого
  скана были изменены на диске (страницы переставлены/заменены вручную)
  позже последней конвертации — так устаревший EPUB видно сразу, не
  сравнивая таймстемпы вручную.
...
- **Rename** — переопределяет отображаемое имя книги (только
  `books.series_name` в БД, файл и папка на диске не трогаются) — единственный
  способ поменять то, что показывается для манги/PDF-книги, поскольку иначе
  её имя серии зашито в метаданные EPUB каждого тома ещё при конвертации.
...
![Управление книгами](docs/screenshots/manage-books-grand-blue.png)

### Проверка целостности библиотеки (только для администраторов)

Settings → Library Integrity запускает явную проверку всех настроенных
библиотек в режиме только для чтения. Проверяется наличие и доступность
проиндексированных файлов, структура EPUB и его spine, чтение HTML-файлов и
сгенерированных обложек, совпадение размера файла с записанным значением,
нахождение пути книги внутри своей библиотеки и наличие на диске
поддерживаемых, но не проиндексированных файлов. Также находятся оставшиеся
после старого удаления или переконвертации файлы обложек, на которые больше не
ссылается ни одна книга. В отчёте ошибки отделены от
предупреждений, а пути показываются только относительно библиотеки. Проверка
ничего не сканирует заново, не исправляет, не изменяет и не удаляет.

![Успешная проверка целостности библиотеки](docs/screenshots/library-integrity-dark.png)

### Server Status (только для администраторов)

Settings → Server Status показывает живые метрики хоста — CPU/RAM/GPU
(загрузка, температура, VRAM), опрашивается каждые 15с, в памяти хранятся
последние 4 часа (сбрасывается при рестарте — это именно живая телеметрия, а
не сохранённая история). Полезно смотреть за загрузкой/температурой GPU во
время долгого пакета OCR-конвертаций.
...
- Закладки: несколько цветных выделений на книгу, заметки и панель для
  перехода, редактирования или удаления
