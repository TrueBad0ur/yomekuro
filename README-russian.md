...
Монтируется один каталог `./library` с четырьмя подпапками внутри: `ranobe/`,
`manga/`, `html/` и `pdf/` (в первых двух — одна папка на серию, внутри
`.epub` файлы; `html/` хранит отдельные `.html` файлы, один файл = одна
книга; `pdf/` — для PDF, которые не манга и не ранобэ, см. "Конвертер"
ниже, туда применяется тот же пайплайн загрузки). Все четыре регистрируются
как отдельные библиотеки и сканируются автоматически при старте. Откройте
http://localhost:8080 и войдите.
...
сгруппированных по библиотекам (Ranobe / Manga / HTML / PDF) в боковой панели.
...
Настройки → Upload manga: выберите библиотеку, перетащите в область загрузки
архив с исходными изображениями страниц или PDF, и укажите имя. Задача
становится в очередь, и её прогресс (текущий том, число страниц) стримится
в лог прямо на той же странице, пока EPUB не будет готов и не появится в
библиотеке.
...
Settings → Upload manga: выберите библиотеку, файл и имя. Файл — либо архив
...
при наведении). Выделение держится на глифах, а не съезжает вбок, благодаря
двум путям позиционирования: страницы с OCR от mokuro размещают каждую строку
по геометрии линий самого детектора; страницы с текстовым слоем PDF — по
собственным координатам PDF.
...
Внутри него четыре подпапки автоматически регистрируются как отдельные
...
library/
  ...
  pdf/      # PDF, которые не манга и не ранобэ — тот же пайплайн загрузки/конвертера
```

Весь том `./library` монтируется на чтение-запись (не `:ro`), потому что
функция загрузки манги распаковывает архивы прямо в `library/manga/` (а PDF —
в ту библиотеку, что выберете, включая `library/pdf/`).
